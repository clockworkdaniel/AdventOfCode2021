...
export const countInstancesOfLetter = (signalPatterns: string[]) : LetterInstances => {
...
export const getEntryValue = ({signalPatterns, digitValues}: Entry) => {
  ...
  const findBySegmenetAndRemoveFromSignalPatterns = (regexString: string, ofLength?: number): string => {
    let letter: string;
    if (!ofLength) {
      letter = signalPatterns.find(sp => new RegExp(regexString, 'g').test(sp) === false)!;
    } else {
      letter = signalPatterns.filter(sp => sp.length === ofLength).find(sp => new RegExp(regexString, 'g').test(sp) === false)!;
    }
    signalPatterns = signalPatterns.filter(sp => sp !== letter);
  ...
  console.log(leftBottom, leftTop)

  digitPatterns[5] = findBySegmenetAndRemoveFromSignalPatterns(`[${leftBottom}|${rightTop}]`);
  ...
  digitPatterns[3] = findBySegmenetAndRemoveFromSignalPatterns(leftBottom); // not working
  digitPatterns[2] = signalPatterns[0];

   console.log(digitPatterns);

  return digitValues.reduce((output, dv) => {
    const i = digitPatterns.findIndex(dp => dp === dv) // need to check for jumbled
    return output + i
  },'')
